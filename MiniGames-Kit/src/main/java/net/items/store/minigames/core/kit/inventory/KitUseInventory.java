...
public class KitUseInventory extends AbstractCustomInventory {

    public KitUseInventory(){
        super(KitInventoryIdentifier.KIT_IDENTIFIER_KIT_USE, "{KIT_NAME}", 27);
    }
    ...
    public void fillDefaultItems(Inventory inventory) {
        ...
        inventory.setItem(8, ItemBuilder.modify().setMaterial(Material.RED_STAINED_GLASS_PANE).setDisplayName("§cSchließen").buildItem());
    ...
    public void fillCustomItems(Player player, Inventory inventory, Object... objects) {
        ...
        inventory.setItem(17, ItemBuilder.modify().setMaterial(Material.GREEN_DYE)
                .setDisplayName(messageManager.getMessage("KitChangeDisplayName"))
                .setLore(Arrays.stream(messageManager.getMessage("KitChangeLore").split("%NL%")).toList())
                .buildItem());
    }
    ...
    public void performInventoryClick(InventoryClickEvent inventoryClickEvent) {
        ...
        if(optionalKit.isPresent()){
            ...
            if(inventoryClickEvent.getCurrentItem().getItemMeta().getDisplayName().equalsIgnoreCase(messageManager.getMessage("KitChangeDisplayName"))){
